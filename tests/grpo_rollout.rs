...
use qwen3_burn::grpo::{
    group_sample, group_sample_cached, group_sample_cached_device,
...
fn group_sample_shapes_and_logprobs() {
    ...
    let prompt = Tensor::<B, 1, Int>::from_data([1i64, 2, 3, 4, 5, 6].as_slice(), &dev).reshape([p, lp]);

    let g = 3usize;
    let rc = RolloutConfig { group_size: g, max_new_tokens: 4, temperature: 1.0, top_p: 0.9, top_k: 0 };
    let eos = [7i64];
    ...
    for s in 0..n {
        assert_eq!(mask[s * roll.gen_len], 1.0, "first generated token must be unmasked");
    }
    ...
    let ids = roll.seq_ids.into_data().to_vec::<i64>().unwrap();
    for &t in &ids {
        assert!((0..32).contains(&t), "token {t} out of vocab");
    }

    println!("rollout OK — N={n} gen_len={} seq_cols={}", roll.gen_len, lp + roll.gen_len);
}
...
fn cached_matches_uncached_greedy() {
    ...
    let rc = RolloutConfig { group_size: 3, max_new_tokens: 6, temperature: 0.0, top_p: 1.0, top_k: 0 };
    let eos = [7i64];

    let a = group_sample(&model, prompt.clone(), &rc, &eos);
    let b = group_sample_cached(&model, prompt, &rc, &eos);

    assert_eq!(a.gen_len, b.gen_len, "gen_len: no-cache {} vs cache {}", a.gen_len, b.gen_len);
    ...
    for (i, (x, y)) in al.iter().zip(bl.iter()).enumerate() {
        assert!((x - y).abs() < 1e-4, "old_logprob[{i}] differs: no-cache {x} vs cache {y}");
    }

    println!("cache==no-cache parity OK (greedy) — gen_len={} N={}", a.gen_len, p * rc.group_size);
}
...
fn shrink_matches_unshrunk_greedy() {
    let dev = Default::default();
    <B as Backend>::seed(&dev, 7); // deterministic random init -> reproducible length spread
    ...
    let prompt = Tensor::<B, 1, Int>::from_data(prompt_ids.as_slice(), &dev).reshape([p, lp]);
    ...
    let rc = RolloutConfig { group_size: g, max_new_tokens: 24, temperature: 0.0, top_p: 1.0, top_k: 0 };
    ...
    assert_eq!(a.gen_len, b.gen_len, "gen_len: unshrunk {} vs shrink {}", a.gen_len, b.gen_len);
    assert_eq!(a.seq_ids.dims(), b.seq_ids.dims(), "seq_ids shape");
    ...
    let lengths: Vec<usize> =
        (0..n).map(|s| (0..gen_len).filter(|&t| am[s * gen_len + t] == 1.0).count()).collect();
    let (minl, maxl) = (*lengths.iter().min().unwrap(), *lengths.iter().max().unwrap());
    let finished_before_end = lengths.iter().filter(|&&l| l < gen_len).count();
    assert!(minl < maxl, "no length variance ({minl}..{maxl}) — shrink would never fire; retune the test");
    assert!(
    ...
    assert!(masked_max < 1e-4, "real-token logprob max-err {masked_max} exceeds 1e-4 (shrink corrupted a kept row)");
}
...
fn device_sample_matches_host_greedy() {
    let dev = Default::default();
    <B as Backend>::seed(&dev, 7); // deterministic init -> reproducible length spread
    ...
    assert_eq!(a.gen_len, b.gen_len, "gen_len: host {} vs device {}", a.gen_len, b.gen_len);
    assert_eq!(a.seq_ids.dims(), b.seq_ids.dims(), "seq_ids shape");

    // (1) seq_ids bit-identical (real tokens AND the EOS padding both paths emit).
    let ai = a.seq_ids.into_data().to_vec::<i64>().unwrap();
    let bi = b.seq_ids.into_data().to_vec::<i64>().unwrap();
    assert_eq!(ai, bi, "device and host produced different token ids (greedy must match)");
    ...
    let lengths: Vec<usize> =
        (0..n).map(|s| (0..gen_len).filter(|&t| am[s * gen_len + t] == 1.0).count()).collect();
    let (minl, maxl) = (*lengths.iter().min().unwrap(), *lengths.iter().max().unwrap());
    assert!(minl < maxl, "no length variance ({minl}..{maxl}) — finished-row pad path not exercised");
    ...
    assert!(maxe < 1e-4, "device vs host raw logp max-err {maxe} exceeds 1e-4");
}
...
fn device_sample_temperature_logp_is_raw() {
    ...
    assert!(maxe < 1e-4, "device temperature logp max-err {maxe} exceeds 1e-4");
}
...
fn canonical_gate_long_context_parity() {
    ...
    let prompt = Tensor::<B, 1, Int>::from_data(prompt_ids.as_slice(), &dev).reshape([p, lp]);
    let rc = RolloutConfig { group_size: 2, max_new_tokens: 24, temperature: 0.0, top_p: 1.0, top_k: 0 };
    let eos = [63i64]; // unlikely id ⇒ generate the full length, exercising long positions

    let a = group_sample(&model, prompt.clone(), &rc, &eos);
    let b = group_sample_cached(&model, prompt, &rc, &eos);

    assert_eq!(a.gen_len, b.gen_len, "long-context gen_len: no-cache {} vs cache {}", a.gen_len, b.gen_len);
    let ai = a.seq_ids.into_data().to_vec::<i64>().unwrap();
    let bi = b.seq_ids.into_data().to_vec::<i64>().unwrap();
    assert_eq!(ai, bi, "long-context: cache vs no-cache token ids diverge (position/mask bug)");
    ...
    println!("canonical gate (long-ctx gen_len={}): per-token logprob max-err={maxe:.2e} mean-err={meane:.2e}", a.gen_len);
    assert!(maxe < 1e-4, "long-context per-token logprob max-err {maxe} exceeds 1e-4 (token-local corruption)");
}
...
fn device_loop_matches_device_greedy() {
    ...
    assert_eq!(b.gen_len, max_new, "device-loop must run the full fixed decode length");

    let g0 = a.gen_len; // reference generated length (<= max_new); compare the common prefix
    assert!(g0 >= 1);

    let ai = a.seq_ids.into_data().to_vec::<i64>().unwrap(); // [n, lp+g0]
    let bi = b.seq_ids.clone().into_data().to_vec::<i64>().unwrap(); // [n, lp+max_new]
    let am = a.completion_mask.into_data().to_vec::<f32>().unwrap(); // [n, g0]
    let bm = b.completion_mask.clone().into_data().to_vec::<f32>().unwrap(); // [n, max_new]
    ...
    for s in 0..n {
        for t in 0..g0 {
            assert_eq!(am[s * g0 + t], bm[s * max_new + t], "completion mask differs at row {s} step {t}");
        ...
        for t in g0..max_new {
            assert_eq!(bm[s * max_new + t], 0.0, "device-loop tail must be masked padding (row {s} step {t})");
        }
    }
    ...
    let lengths: Vec<usize> =
        (0..n).map(|s| (0..g0).filter(|&t| am[s * g0 + t] == 1.0).count()).collect();
    let (minl, maxl) = (*lengths.iter().min().unwrap(), *lengths.iter().max().unwrap());
    assert!(minl < maxl, "no length variance ({minl}..{maxl}) — finished-row pad path not exercised; retune eos");

    // (4) STRUCTURAL no-per-step-host-read check: the driver source must contain ZERO into_data/to_vec.
    let src = std::fs::read_to_string(concat!(env!("CARGO_MANIFEST_DIR"), "/src/grpo/rollout.rs")).unwrap();
    let body = {
        let start = src.find("pub fn group_sample_cached_device_loop").expect("driver fn present");
        let after = &src[start..];
        // up to the next top-level `pub fn ` (the following driver) — the whole function body.
        let end = after[1..].find("\npub fn ").map(|i| i + 1).unwrap_or(after.len());
        &after[..end]
    };
    let n_host_reads = body.matches("into_data").count() + body.matches("to_vec").count();
    assert_eq!(n_host_reads, 0, "device-loop driver must have NO per-step host read (found {n_host_reads} into_data/to_vec)");
    ...
    assert!(maxe < 1e-4, "device-loop vs host raw logp max-err {maxe} exceeds 1e-4");
}
...
fn static_matches_device_loop_greedy() {
    ...
    assert_eq!(a.gen_len, max_new, "loop driver must run the full fixed decode length");
    assert_eq!(b.gen_len, max_new, "static driver must run the full fixed decode length");
    assert_eq!(a.seq_ids.dims(), b.seq_ids.dims(), "seq_ids shape: loop {:?} vs static {:?}", a.seq_ids.dims(), b.seq_ids.dims());

    // (1) seq_ids BIT-IDENTICAL (real tokens AND the device-emitted EOS padding).
    let ai = a.seq_ids.into_data().to_vec::<i64>().unwrap();
    let bi = b.seq_ids.into_data().to_vec::<i64>().unwrap();
    assert_eq!(ai, bi, "device-pos static vs host-`t` loop produced different token ids (greedy must match)");

    // (2) completion_mask BIT-IDENTICAL.
    let am = a.completion_mask.into_data().to_vec::<f32>().unwrap();
    let bm = b.completion_mask.into_data().to_vec::<f32>().unwrap();
    assert_eq!(am, bm, "completion masks differ (device-pos mask scatter != host-`t` slice_assign)");
    ...
    let lengths: Vec<usize> =
        (0..n).map(|s| (0..max_new).filter(|&t| am[s * max_new + t] == 1.0).count()).collect();
    let (minl, maxl) = (*lengths.iter().min().unwrap(), *lengths.iter().max().unwrap());
    assert!(minl < maxl, "no length variance ({minl}..{maxl}) — retune eos");
    assert_eq!(maxl, max_new, "no row reached full length — the mask boundary isn't exercised at all positions");
    ...
    let src = std::fs::read_to_string(concat!(env!("CARGO_MANIFEST_DIR"), "/src/grpo/rollout.rs")).unwrap();
    let body = {
        let start = src.find("pub fn group_sample_cached_device_static").expect("static driver present");
        let after = &src[start..];
        let end = after[1..].find("\npub fn ").map(|i| i + 1).unwrap_or(after.len());
        &after[..end]
    };
    let n_host_reads = body.matches("into_data").count() + body.matches("to_vec").count();
    assert_eq!(n_host_reads, 0, "static driver must have NO per-step host read (found {n_host_reads})");
    let n_per_step_slice_assign =
        body.matches("(lp + t)").count() + body.matches("t..t + 1").count() + body.matches("t..t+1").count();
    assert_eq!(
    ...
    assert!(body.contains("select_assign"), "static driver must scatter per-step writes by device pos");
    ...
    assert!(maxe < 1e-4, "device-pos static vs host-`t` loop raw logp max-err {maxe} exceeds 1e-4");
}
