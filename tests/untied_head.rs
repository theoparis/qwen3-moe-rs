...
fn untied_forward_runs_and_differs_from_tied() {
    let dev = Device::flex();
    let ids = Tensor::<1, Int>::from_data([1i64, 2, 3, 4].as_slice(), &dev).reshape([1, 4]);
    ...
    assert!(
        maxabs > 1e-3,
        "untied logits should differ from tied (separate head not used?) maxabs={maxabs}"
    );
