...
use burn::tensor::{Bool, DType, Device, Int, Tensor};
use qwen3_burn::Qwen3Config;
use qwen3_burn::grpo::{RolloutConfig, group_sample, group_sample_padded};

fn int_ids<const D: usize>(t: Tensor<D, Int>) -> Vec<i64> {
    t.cast(DType::I64).into_data().to_vec::<i64>().unwrap()
}

fn tiny_model(dev: &Device) -> qwen3_burn::Qwen3ForCausalLM {
    Qwen3Config::new()
        .with_vocab_size(32)
        .with_hidden_size(64)
...
fn left_pad_is_logit_invariant() {
    ...
    let real = [3i64, 7, 1, 9];
    let lp = real.len();
    let unpadded = Tensor::<1, Int>::from_data(real.as_slice(), &dev).reshape([1, lp]);
    ...
    let padded = Tensor::<1, Int>::from_data(padded_ids.as_slice(), &dev).reshape([1, l]);
    ...
    let mask = Tensor::<1, Bool>::from_data(mask_v.as_slice(), &dev).reshape([1, l]);

    // RoPE positions = cumsum(mask) - 1, pad clamped to 0: [0, 0, 0, 1, 2, 3]
    let mut pos_v = vec![0i64; pad];
    pos_v.extend(0..lp as i64);
    let pos = Tensor::<1, Int>::from_data(pos_v.as_slice(), &dev).reshape([1, l]);
    ...
    assert!(
        ...
        "padded forward produced non-finite logits (NaN poisoning)"
    );
    let mut maxabs = 0f32;
    for t in 0..lp {
        for c in 0..v {
    ...
    assert!(
        maxabs < 1e-3,
        "left-pad changed the real-token logits: max|diff| = {maxabs}"
    );
...
fn padded_rollout_matches_unpadded_greedy() {
    ...
    // unpadded reference
    let unpadded = Tensor::<1, Int>::from_data(real.as_slice(), &dev).reshape([1, lp_real]);
    ...
    padded_ids.extend_from_slice(&real);
    let lp = pad + lp_real;
    let padded = Tensor::<1, Int>::from_data(padded_ids.as_slice(), &dev).reshape([1, lp]);
    let b = group_sample_padded(&model, padded, &[lp_real], &rc, &eos);

    assert_eq!(
        a.gen_len, b.gen_len,
        "gen_len differs: unpadded {} vs padded {}",
        a.gen_len, b.gen_len
    );
    ...
    assert_eq!(
        a_comp, b_comp,
        "left-pad changed the greedy completion: {a_comp:?} vs {b_comp:?}"
    );
    ...
    for (i, (x, y)) in al.iter().zip(bl.iter()).enumerate() {
        assert!(
            (x - y).abs() < 1e-3,
            "old_logprob[{i}] differs: unpadded {x} vs padded {y}"
        );
    }
    println!(
        "padded-rollout parity OK (greedy) — gen_len={} completion={:?}",
        a.gen_len, a_comp
    );
}
...
fn cached_prefill_is_left_pad_safe() {
    ...
    let padded = Tensor::<1, Int>::from_data(ids.as_slice(), &dev).reshape([1, lp]);
    let mut mv = vec![false; pad];
    mv.extend(std::iter::repeat(true).take(real.len()));
    let mask = Tensor::<1, Bool>::from_data(mv.as_slice(), &dev).reshape([1, lp]);
    let mut pv = vec![0i64; pad];
    pv.extend(0..real.len() as i64);
    let pos = Tensor::<1, Int>::from_data(pv.as_slice(), &dev).reshape([1, lp]);
    ...
    assert!(
        ...
        "cached prefill produced non-finite logits (NaN poisoning)"
    );
    let mut maxabs = 0f32;
    for t in pad..lp {
        for k in 0..v {
    ...
    assert!(
        maxabs < 1e-3,
        "cached prefill != no-cache on real tokens: max|diff| = {maxabs}"
    );
