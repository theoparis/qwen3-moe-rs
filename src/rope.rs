...
pub fn rope_freqs<B: Backend>(head_dim: usize, theta: f64, device: &B::Device) -> Tensor<B, 1> {
    let half_dim = head_dim / 2;
    let freq_seq: Vec<f32> = (0..half_dim)
        .map(|i| 1.0 / (theta as f32).powf(2.0 * i as f32 / head_dim as f32))
        .collect();
    Tensor::<B, 1>::from_floats(freq_seq.as_slice(), device)
}
...
pub fn compute_rope_embeddings_pre<B: Backend>(
    positions: Tensor<B, 2, Int>,
    freqs: Tensor<B, 1>,
) -> (Tensor<B, 4>, Tensor<B, 4>) {
    let positions_float = positions.float();
...
pub fn apply_rope_partial<B: Backend>(
    ...
    let q_rot = q.clone().slice([0..batch, 0..seq, 0..q_heads, 0..rotary_dim]);
    let q_pass = q.slice([0..batch, 0..seq, 0..q_heads, rotary_dim..q_head_dim]);
    let k_rot = k.clone().slice([0..batch, 0..seq, 0..k_heads, 0..rotary_dim]);
    ...
    (Tensor::cat(vec![q_rotated, q_pass], 3), Tensor::cat(vec![k_rotated, k_pass], 3))
}
