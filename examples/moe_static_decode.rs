...
fn bench_per_step(
    ...
    assert!(
        lp + decode_tokens <= total,
        "lp+decode_tokens {} > sd.capacity {total}",
        lp + decode_tokens
    );
    ...
    let mut next: Tensor<2, Int> = logits
        .slice([0..1, (lp - 1)..lp, 0..vocab])
        .reshape([1, vocab])
        .argmax(1)
        .reshape([1, 1]);
...
fn run() -> Result<(), String> {
    ...
    let eager_ids: Vec<i64> = eager
        .cast(DType::I64)
        .to_data()
        .to_vec()
        .map_err(|e| format!("read eager: {e:?}"))?;
    ...
    let stat_ids: Vec<i64> = stat
        .cast(DType::I64)
        .to_data()
        .to_vec()
        .map_err(|e| format!("read static: {e:?}"))?;
    ...
    let eager_txt = tokenizer
        ...
        .unwrap_or_default();
    let stat_txt = tokenizer
        .decode(&stat_ids.iter().map(|&x| x as u32).collect::<Vec<_>>())
        .unwrap_or_default();
    ...
    if identical {
        println!(
            "  ==> GATE PASS: static decode is GREEDY-TOKEN-IDENTICAL to eager (bf16 parity holds)."
        );
    } else {
        println!(
            "  ==> GATE FAIL: first divergence at absolute pos {first_div:?} (completion idx {:?}).",
    ...
    for (label, sd) in [
        ("oracle (decode_topk_pre, materializing)", &sd_oracle),
        ("FUSED  (decode_topk_fused, lever c)", &sd_fused),
    ] {
        ...
        println!(
            "    median {ms:.1} ms/tok | mean {:.1} | min {:.1}",
            mean(steady),
            steady.iter().cloned().fold(f64::INFINITY, f64::min)
        );
        println!(
            "    => {tok_s:.3} tok/s | {gb_tok:.2} GB/tok (experts {:.2}+attn {:.2}+head {:.2}) | {eff:.1} GB/s eff | {pct:.0}% peak",
            be / 1e9,
            ba / 1e9,
            bh / 1e9
        );
    ...
    if tok_identical {
        println!(
            "    ==> TOKEN-IDENTICAL: fused gather-GEMV == materializing oracle on the real bf16 model."
        );
    } else {
        println!(
            "    ==> MISMATCH at decode idx {first_div:?} (fused diverged from the oracle decode)."
        );
    }
    ...
    println!(
        "    oracle (materializing select) : {o_toks:6.3} tok/s   {:6.1} GB/s   {o_pct:3.0}% peak",
        gb_tok * o_toks
    );
    println!(
        "    FUSED  (gather-GEMV, lever c) : {f_toks:6.3} tok/s   {:6.1} GB/s   {f_pct:3.0}% peak",
        gb_tok * f_toks
    );
    println!(
        "    => fused speedup over materializing oracle: {speedup:.2}x  ({} the no-materialization win)",
        if speedup > 1.03 {
            "REALIZED"
        } else {
            "did NOT realize"
