...
use burn::tensor::{Bool, Device, IndexingUpdateOp, Int, Tensor, TensorPrimitive};
...
struct BucketGraph {
    ...
    tok_buf: Option<Tensor<2, Int>>,
    logp_buf: Option<Tensor<2>>,
    mask_buf: Option<Tensor<2>>,
    pos: Option<Tensor<1, Int>>,
    finished: Option<Tensor<2, Int>>,
    last_buf: Option<Tensor<2>>,
    lo: Option<Tensor<1, Int>>, // left-pad column count (= bucket - L); read by the captured mask
    pad: Tensor<2, Int>,
    cache: ModelCache,
    freqs: Tensor<1>,
...
impl BucketGraph {
    ...
    fn capture(
        ...
        assert!(
            warmup < max_new,
            "warmup {warmup} must be < max_new {max_new} (OOB guard)"
        );
        ...
        let cap_ids: Vec<i64> = (0..(p * bucket) as i64)
            .map(|i| (i * 131 + 17) % v as i64)
            .collect();
        let prompt = Tensor::<1, Int>::from_data(cap_ids.as_slice(), &device).reshape([p, bucket]);
        ...
        let lo = Some(Tensor::<1, Int>::zeros([1], &device));

        // canonical prefill (pad_len=0, no mask): fill KV cols 0..bucket + initial logits -> last_buf.
        let prefill = |cache: &mut ModelCache, last_buf: &mut Option<Tensor<2>>| {
            let pos0 = Tensor::<1, Int>::arange(0..bucket as i64, &device)
                .unsqueeze_dim::<2>(0)
                .repeat(&[n, 1]);
            let logits = model.forward_with_cache(prompt_rep.clone(), None, pos0, cache);
            let prefill_last = logits
                .slice([0..n, (bucket - 1)..bucket, 0..v])
                .reshape([n, v]);
        ...
        for (i, (b, a)) in va_before.iter().zip(va_after.iter()).enumerate() {
            assert_eq!(
                b, a,
                "VA-STABILITY VIOLATION: persistent buffer '{}' relocated across capture (bucket {bucket})",
                va_labels[i]
            );
        }
    ...
    fn dispatch(
        ...
        assert_eq!(
            p, self.p,
            "dispatch prompt batch must match the bucket graph"
        );
        assert!(
            l <= self.bucket,
            "prompt length {l} exceeds bucket {}",
            self.bucket
        );
        ...
        let prompt_rep_real = prompt
            .unsqueeze_dim::<3>(1)
            .repeat(&[1, self.g, 1])
            .reshape([n, l]);
        let prompt_rep = if pad_len > 0 {
            let pad_block = Tensor::<2, Int>::full([n, pad_len], self.eos0, device);
        ...
        let attn_mask =
            Tensor::<1, Bool>::from_data(mask_flat.as_slice(), device).reshape([n, bucket]);
        ...
        self.cache.reset_for_replay();
        {
            let pos0 = Tensor::<1, Int>::arange(0..bucket as i64, device)
                .unsqueeze_dim::<2>(0)
                .repeat(&[n, 1]);
            let mask_in = if lo_on && pad_len > 0 {
                Some(attn_mask)
            } else {
                None
            };
            let logits =
                model.forward_with_cache(prompt_rep.clone(), mask_in, pos0, &mut self.cache);
            let prefill_last = logits
                .slice([0..n, (bucket - 1)..bucket, 0..v])
                .reshape([n, v]);
        ...
        self.tok_buf = Some(
            self.tok_buf
                .take()
                .unwrap()
                .slice_assign([0..n, 0..bucket], prompt_rep),
        );
...
fn completion_ids(r: &Rollouts) -> Vec<i32> {
...
fn run_abort_uaf() {
    ...
    println!(
        "  bucket A (lp={bucket_a}) captured + sealed into shared pool ({} KB)",
        a_bytes / 1024
    );

    // Reference completion from bucket A BEFORE the aborted capture.
    let a_ids: Vec<i64> = (0..(p * bucket_a) as i64)
        .map(|i| (i * 131 + 17) % vocab as i64)
        .collect();
    let prompt_a = Tensor::<1, Int>::from_data(a_ids.as_slice(), &device).reshape([p, bucket_a]);
    let ref_ids = bg_a
    ...
    block_sync(&client);
    println!(
        "  bucket A reference completion recorded ({} ids)",
        ref_ids.len()
    );
    ...
    let panicked = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        ...
        let mut buf = Some(Tensor::<1>::zeros([bucket_b], &device));
    ...
    assert!(
        panicked.is_err(),
        "bucket B capture was expected to PANIC/abort but did not"
    );
    block_sync(&client);
    println!(
        "  bucket B (lp={bucket_b}) capture into the shared pool deliberately ABORTED (panic caught)"
    );
...
fn main() {
    ...
    // -------------------------------------------------------------------------------------------------
    println!(
        "--- (1) per-bucket: CAPTURED greedy (bucket graph, pad_len=0) == eager static decode ---"
    );
    let pool = client.capture_pool();
    let mut graphs: std::collections::HashMap<usize, BucketGraph> =
        std::collections::HashMap::new();
    let mut shared_pool_bytes = 0u64;
    let mut all_pass = true;
    for &b in &buckets {
        let (bg, bytes) =
            BucketGraph::capture(&model, &pool, p, g, b, max_new, vocab, &eos, warmup);
    ...
    for &b in &buckets {
        let ids: Vec<i64> = (0..(p * b) as i64)
            .map(|i| (i * 131 + 17) % vocab as i64)
            .collect();
        let prompt = Tensor::<1, Int>::from_data(ids.as_slice(), &device).reshape([p, b]);
        ...
        assert!(
            pass,
            "bucket {b} captured decode diverged from eager static"
        );
    }
    println!(
        "  => {}\n",
        if all_pass {
            "PASS: every bucket's captured decode is bit-identical to eager static"
        } else {
            "FAIL"
        }
    );
    ...
    // -------------------------------------------------------------------------------------------------
    println!(
        "--- (2) left-pad: real length L -> bucket B (lo mask) ~= decode at true length L (FP/argmax-robust) ---"
    );
    let cases: [(usize, usize); 3] = [(20, 32), (10, 16), (50, 64)]; // (L, bucket B) with L != B
    let mut leftpad_pass = true;
    for (l, b) in cases {
        assert!(buckets.contains(&b));
        let ids: Vec<i64> = (0..(p * l) as i64)
            .map(|i| (i * 97 + 5) % vocab as i64)
            .collect();
        let prompt = Tensor::<1, Int>::from_data(ids.as_slice(), &device).reshape([p, l]);

        // reference: decode at the TRUE length L (eager static, no padding).
        let rc_l = RolloutConfig {
            group_size: g,
            max_new_tokens: max_new,
            temperature: 0.0,
            top_p: 1.0,
        ...
        assert!(
            pass,
            "left-pad to bucket {b} did not match the true-length-{l} completion up to FP/argmax robustness (on={on_frac:.3} off={off_frac:.3})"
        );
    }
    println!(
        "  => {}\n",
        if leftpad_pass {
            "PASS: left-pad to a bucket matches the true-length decode up to FP/argmax robustness (bit-identical only at pad_len=0); the lo mask is load-bearing"
        } else {
            "FAIL"
        }
    );
    ...
    let mut separate_each = Vec::new();
    {
        ...
        for &b in &buckets {
            ...
            let (bg, bytes) =
                BucketGraph::capture(&model, &sep_pool, p, g, b, max_new, vocab, &eos, warmup);
    ...
    println!(
        "  K-in-1 shared pool high-water: {:>8} KB",
        shared_pool_bytes / 1024
    );
    ...
    println!(
        "  => {}\n",
        if mem_pass {
            "PASS: K buckets in 1 pool ~= 1 (largest) bucket's arena, not K x"
        } else {
            "FAIL (see numbers)"
        }
    );
    ...
    println!("=== P4 SUMMARY ===");
    println!(
        "  (1) per-bucket capture bit-identical to eager static: {}",
        if all_pass { "PASS" } else { "FAIL" }
    );
    println!(
        "  (2) left-pad to a bucket preserves the completion:    {}",
        if leftpad_pass { "PASS" } else { "FAIL" }
    );
    println!(
        "  (3) K buckets share ~1 bucket's arena (shared pool):  {}",
