...
fn main() {
    ...
    let xdata: Vec<f32> = (0..m * k)
        .map(|i| (((i * 1103515245 + 12345) % 1000) as f32 / 1000.0 - 0.5) * 1.1)
        .collect();
    let wdata: Vec<f32> = (0..k * n)
        .map(|i| (((i * 1664525 + 1013904223) % 1000) as f32 / 1000.0 - 0.5) * 0.024)
        .collect();

    let x = Tensor::<2>::from_data(TensorData::new(xdata.clone(), [m, k]), &dev);
    let w = Tensor::<2>::from_data(TensorData::new(wdata.clone(), [k, n]), &dev);
    ...
    let xc = Tensor::<2>::from_data(TensorData::new(xdata, [m, k]), &cdev);
    let wc = Tensor::<2>::from_data(TensorData::new(wdata, [k, n]), &cdev);
    ...
    let diff_bl = (y_batched.clone() - y_loop.clone())
        .abs()
        .max()
        .into_scalar();
