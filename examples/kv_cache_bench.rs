...
fn main() {
    ...
    for &t in &[128usize, 512] {
        ...
        for &(name, capacity) in &[
            ("cat   (legacy O(T^2))", None::<usize>),
            ("static (Phase 2)", Some(cap)),
        ] {
            let mut cache = match capacity {
                Some(c) => KVCache::<B>::with_capacity(c),
                None => KVCache::<B>::new(),
            };
            let k0 = Tensor::<4>::random([n, lp, kvh, hd], Distribution::Normal(0.0, 1.0), &device);
            let v0 = Tensor::<4>::random([n, lp, kvh, hd], Distribution::Normal(0.0, 1.0), &device);
            ...
            for _ in 0..t {
                let k =
                    Tensor::<4>::random([n, 1, kvh, hd], Distribution::Normal(0.0, 1.0), &device);
                let v =
                    Tensor::<4>::random([n, 1, kvh, hd], Distribution::Normal(0.0, 1.0), &device);
            ...
            println!(
                "  T={t:4} {name}: {ms:7.1} ms total  ({:.3} ms/step)",
                ms / t as f64
            );
            results.push(ms);
        }
        println!(
            "  T={t:4} -> static is {:.2}x the cat time\n",
            results[1] / results[0]
        );
    }
}
