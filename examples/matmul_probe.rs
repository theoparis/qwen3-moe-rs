...
fn rowdiff3(t: &Tensor<B, 3>) -> f32 {
    let [b, s, n] = t.dims();
    assert_eq!(b, 2);
    let r0 = t.clone().slice([0..1, 0..s, 0..n]);
    let r1 = t.clone().slice([1..2, 0..s, 0..n]);
    (r0 - r1).abs().max().cast(DType::F32).into_data().as_slice::<f32>().map(|x| x[0]).unwrap_or(f32::NAN)
}

fn case(seq: usize, k: usize, n: usize) {
    let device = CudaDevice::default();
    let x1 = Tensor::<B, 3>::random([1, seq, k], Distribution::Normal(0.0, 1.0), &device);
    let x = x1.repeat(&[2, 1, 1]); // identical rows
    let w = Tensor::<B, 2>::random([k, n], Distribution::Normal(0.0, 0.02), &device);
    let wb = w.clone().unsqueeze::<3>(); // [1, k, n] -> broadcast
    let out = x.matmul(wb);
    println!("matmul [2,{seq},{k}] @ [{k},{n}]  |r0-r1| = {:.6}  (must be 0)", rowdiff3(&out));
}
...
fn case_2d(seq: usize, k: usize, n: usize) {
    ...
    println!("2D-flat [2,{seq},{k}] @ [{k},{n}]  |r0-r1| = {:.6}  (must be 0)", rowdiff3(&out));
}

fn scalar(t: Tensor<B, 1>) -> f32 {
    t.cast(DType::F32).into_data().as_slice::<f32>().map(|x| x[0]).unwrap_or(f32::NAN)
}
...
fn case_2d_bf16(seq: usize, k: usize, n: usize) {
    ...
    let bf2 = x2.cast(DType::BF16).matmul(w.cast(DType::BF16)).cast(DType::F32);
    // G2: batch-safety.
...
fn case_g6_residual() {
    ...
    let inc = Tensor::<B, 1>::from_floats([1e-5f32], &device);
    let v_f32 = scalar(one.clone() + inc.clone());
    let v_bf16 = scalar((one.cast(DType::BF16) + inc.cast(DType::BF16)).cast(DType::F32));
    let out = Tensor::<B, 2>::from_floats([[1.0f32, 2.0]], &device)
        .cast(DType::BF16)
        .matmul(Tensor::<B, 2>::from_floats([[1.0f32], [1.0]], &device).cast(DType::BF16))
        .cast(DType::F32);
    println!(
