...
fn main() {
    ...
    assert_eq!((cfg.partial_rotary_factor * cfg.head_dim as f64) as usize, 64);
    ...
    let x = Tensor::<B, 3>::random([1, 8, 2048], Distribution::Normal(0.0, 1.0), &device);
    ...
    let values = out.into_data().to_vec::<f32>().expect("read full-attention output");
    assert!(
