...
use burn::tensor::{Device, Distribution, Int, Tensor, activation::silu};

type B = Cuda;

fn sync(device: &CudaDevice) {
    // Force a host sync (CubeCL is async): read a tiny scalar.
    let _ = Tensor::<1>::zeros([1], device).sum().into_scalar();
}
...
fn main() {
    ...
    for &c in &[1usize, 8, 64] {
        let a = Tensor::<3>::random([e, c, h], Distribution::Normal(0.0, 1.0), &device);
        let w = Tensor::<3>::random([e, h, i], Distribution::Normal(0.0, 1.0), &device);
        ...
        println!(
            ...
            if rel < 1e-2 {
                "MATCH — batched GEMM is SAFE on sm_121"
            } else {
                "CORRUPT — must use the 2-D loop / custom kernel"
            }
        );
    }
    ...
    for &c in &[1usize, 64] {
        ...
        println!(
            "  C={c:3}: batched {:.2} ms   per-expert-loop {:.2} ms   -> {:.1}x faster",
            tb * 1e3,
            tl * 1e3,
            tl / tb
        );
    ...
    for &c in &[1usize, 32] {
        ...
        let gate_stack = Tensor::<3>::random([e, h, i], Distribution::Normal(0.0, 0.02), &device);
        let up_stack = Tensor::<3>::random([e, h, i], Distribution::Normal(0.0, 0.02), &device);
        let down_stack = Tensor::<3>::random([e, i, h], Distribution::Normal(0.0, 0.02), &device);
        let x = Tensor::<2>::random([c, h], Distribution::Normal(0.0, 1.0), &device);
        let gate_w = Tensor::<2>::random([c, e], Distribution::Default, &device); // routing weights [C,E]
        sync(&device);
        ...
        let t0 = Instant::now();
        for _ in 0..iters {
            let mut acc = Tensor::<2>::zeros([c, h], &device);
            for ei in 0..e {
                let g = gate_stack
                    .clone()
        ...
        println!(
            "  C={c:3}: oracle {:.2} ms   fast(pre-stacked) {:.2} ms   -> {:.1}x faster",
            to * 1e3,
            tf * 1e3,
            to / tf
        );
    ...
    for &tt in &[256usize, 2048] {
        ...
        let up_stack = Tensor::<3>::random([e, h, i], Distribution::Normal(0.0, 0.02), &device);
        let down_stack = Tensor::<3>::random([e, i, h], Distribution::Normal(0.0, 0.02), &device);
        let x = Tensor::<2>::random([tt, h], Distribution::Normal(0.0, 1.0), &device);
        // random routing (timing only): top-k expert ids in [0,E)
        let sel_idx =
            Tensor::<2>::random([tt, 8], Distribution::Uniform(0.0, e as f64), &device).int(); // [T,k]
        sync(&device);
        let iters = 5;
        ...
        let t0 = Instant::now();
        for _ in 0..iters {
            let mut acc = Tensor::<2>::zeros([tt, h], &device);
            for ei in 0..e {
                let g = gate_stack
                    .clone()
                    .slice([ei..ei + 1, 0..h, 0..i])
        ...
        for _ in 0..iters {
            ...
            let atok = Tensor::<1, Int>::arange(0..tt as i64, &device)
                .reshape([tt, 1])
                .repeat(&[1, 8])
                .reshape([nn]);
            let oh = ae
                .clone()
                .reshape([nn, 1])
                .equal(Tensor::<1, Int>::arange(0..e as i64, &device).reshape([1, e]))
                .int();
            let pos = oh
                .cumsum(0)
                .gather(1, ae.clone().reshape([nn, 1]))
                .reshape([nn])
                .add_scalar(-1i64);
            let over = pos.clone().greater_equal_elem(cap as i64);
            let dest = (ae.mul_scalar(cap as i64) + pos).mask_fill(over, (e * cap) as i64);
            let tok_buf = Tensor::<1, Int>::zeros([e * cap + 1], &device).select_assign(
                0,
                dest,
                atok,
            ...
            let y = (silu(xe.clone().matmul(gate_stack.clone())) * xe.matmul(up_stack.clone()))
                .matmul(down_stack.clone()); // [E,C,H]
            let acc = Tensor::<2>::zeros([tt, h], &device).select_assign(
                0,
                tokens,
                y.reshape([e * cap, h]),
                burn::tensor::IndexingUpdateOp::Add,
        ...
        println!(
            "  T={tt:5} C={cap:4} (E*C/E*T = {:.0}% of dense): oracle {:.1} ms   on-device {:.1} ms   -> {:.1}x faster",
            100.0 * (e * cap) as f64 / (e * tt) as f64,
            to * 1e3,
            td * 1e3,
            to / td
        );
    ...
    println!(
        "  * TRUE per-expert-batched [E,C,H]@[E,H,I] is BIT-EXACT on sm_121 (only the BROADCAST"
    );
    println!(
        "    [B,S,K]@[1,K,N] case corrupts); the NAIVE stacked-batched full pipeline is NOT faster"
    );
    println!(
        "    (~0.8x) — repeat()'ing x to all E experts negates the win (computes full E×dense)."
    );
    println!(
        "  * forward_routed (HOST token-routing, only the touched experts): 2.7x end-to-end on the real"
    );
    println!("    15B + 30B with byte-identical output. Best for single-stream decode (T=1).");
    println!(
        "  * forward_routed_ondevice (ON-DEVICE dispatch: arange-onehot + cumsum + scatter, capacity-C"
    );
    println!(
        "    [E,C,H] batched, no host sync, CUDA-graph-friendly): ~11x above at the BATCHED rollout shape"
    );
    println!(
        "    (C≈1.5kT/E ⇒ 9% of dense). Win grows with T; at T=1 it is full-dense (use the host path)."
    );
    println!(
        "  * Exact-no-drop + compact + fixed-shape together still need a custom grouped-GEMM kernel;"
    );
    println!(
        "    this capacity-padded path is the standard-op stand-in (C<T ⇒ guard overflow for GRPO)."
    );
}
