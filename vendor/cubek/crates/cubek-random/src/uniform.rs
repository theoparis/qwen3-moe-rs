...
use crate::{
    RandomFamily, lcg_step, random_with_seed_handle, taus_step_0, taus_step_1, taus_step_2,
...
/// CUDA-graph-capturable uniform generator (component C3, option (c)).
///
/// Identical to [`random_uniform`] except the 4 seeds are read from a caller-owned DEVICE buffer
/// `seeds` (`N_SEEDS` `u32`) instead of being drawn on the host per call. The buffer pointer is what
/// the captured kernel node bakes, so writing fresh seeds into that SAME buffer (via
/// [`ComputeClient::write_to_handle`]) before each `replay()` decorrelates the draws across replays —
/// the device-seed fix for stochastic capture. `seeds` MUST be allocated OUTSIDE any captured region
/// and outlive the graph (alloc-in-capture is the capture-arena's job).
pub fn random_uniform_with_seeds<R: Runtime>(
    client: &ComputeClient<R>,
    lower_bound: f32,
    upper_bound: f32,
    out: TensorHandleRef<R>,
    ...
    random_with_seed_handle::<UniformFamily, R>(
        client,
        Uniform {
            lower_bound,
            upper_bound,
