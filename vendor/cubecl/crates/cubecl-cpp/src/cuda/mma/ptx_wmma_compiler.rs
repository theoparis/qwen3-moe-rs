...
pub(super) fn compile_scaled_mma<D: Dialect>(
    ...
    let frag_a = (0..a_regs)
        .map(|i| as_const_ty(format!("{frag_a}[{i}]"), ab_ty))
        .collect::<Vec<_>>();
    let frag_b = (0..b_regs)
        .map(|i| as_const_ty(format!("{frag_b}[{i}]"), ab_ty))
        .collect::<Vec<_>>();
    let frag_c_var = frag_c;
    let frag_c_tmp = scaled_mma_acc_tmp(frag_c);
    let frag_c = (0..cd_regs)
        .map(|i| format!("{frag_c_tmp}[{i}]"))
        .collect::<Vec<_>>();
    let frag_d_tmp = scaled_mma_acc_tmp(frag_d);
    let frag_d_args = (0..cd_regs)
        .map(|i| format!("{frag_d_tmp}[{i}]"))
        .collect::<Vec<_>>();
    let fragments =
        comma_separated(frag_a.into_iter().chain(frag_b).chain(frag_c).chain(frag_d_args));
    let frag_d_assignments = (0..cd_regs)
        .map(|i| {
            format!(
                "{} = {frag_d_tmp}[{i}];",
                scaled_mma_acc_out_reg(frag_d, i, cd_regs)
            )
        })
        .collect::<Vec<_>>()
        .join("\n");
...
fn scaled_mma_acc_tmp<D: Dialect>(frag: &Variable<D>) -> String {
    match frag {
        Variable::LocalArray(id, ..) => format!("mma_scaled_acc_{id}"),
        _ => "mma_scaled_acc".to_string(),
    }
}

fn scaled_mma_acc_out_reg<D: Dialect>(
    frag: &Variable<D>,
    scalar_idx: usize,
    _scalar_count: usize,
) -> String {
    let vec = frag.item().vectorization;
    if vec > 1 {
        format!("{frag}[{}].i_{}", scalar_idx / vec, scalar_idx % vec)
    } else {
        format!("{frag}[{scalar_idx}]")
    }
}

fn scaled_mma_acc_in_reg<D: Dialect>(frag: &Variable<D>, scalar_idx: usize) -> String {
    let vec = frag.item().vectorization;
    if vec > 1 {
        format!("{frag}[{}].i_{}", scalar_idx / vec, scalar_idx % vec)
    } else {
