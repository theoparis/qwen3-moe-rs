...
pub fn kernel_scaled<A: CubePrimitive, B: CubePrimitive, CD: Numeric, S: CubePrimitive>(
    ...
    for i in 0..line_count_d {
        let reg = registers_d[i];
        #[unroll]
        for j in 0..line_size_d {
            let n_elem = i * line_size_d + j;
            let (row, col) =
                def.position_of_nth(lane_id, n_elem as u32, MatrixIdent::Accumulator);
            out[row as usize * size_n + col as usize] = reg[j];
        }
    }
}

#[cube(launch)]
pub fn kernel_scaled_fp4_e4m3_factor_4(
    ...
    #[unroll]
    for i in 0..scales_count {
        scales_register_a[i] = scales_a[scales_idx_a as usize * 4usize + i];
    }
    let scales_idx_b = def.scales_index(lane_id, MatrixIdent::B);
    #[unroll]
    for i in 0..scales_count {
        scales_register_b[i] = scales_b[scales_idx_b as usize * 4usize + i];
    }
...
pub fn test_cmma_scaled_fp4_e4m3_factor_4<R: Runtime>(
    client: ComputeClient<R>,
    _cube_dimensions: CubeDim,
) {
    type AB = e2m1x2;
    type S = e4m3;
    ...
    let lhs_data: Vec<f32> = (0..m)
        .flat_map(|row| {
            (0..k).map(move |col| {
                e2m1::from_bits(((row * 3 + col * 5) % 15 + 1) as u8).to_f32()
            })
        })
        .collect();
    let lhs = e2m1x2::from_f32_slice(&lhs_data);
    let rhs_data: Vec<f32> = (0..n)
        .flat_map(|col| {
            (0..k).map(move |row| {
                e2m1::from_bits(((row * 7 + col * 11) % 15 + 1) as u8).to_f32()
            })
        })
        .collect();
    let rhs = e2m1x2::from_f32_slice(&rhs_data);

    let scale_vals = [0.5, 0.75, 1.0, 1.25, 1.5, 1.75, 2.0];
    let lhs_scales_data: Vec<S> = (0..m)
        .flat_map(|row| {
            (0..scales_factor).map(move |block| {
                e4m3::from_f32(scale_vals[(row * 5 + block * 3) % scale_vals.len()])
            })
        })
        .collect();
    let rhs_scales_data: Vec<S> = (0..n)
        .flat_map(|col| {
            (0..scales_factor).map(move |block| {
                e4m3::from_f32(scale_vals[(col * 3 + block * 2 + 1) % scale_vals.len()])
            })
        })
        .collect();
    ...
    unsafe {
        kernel_scaled_fp4_e4m3_factor_4::launch::<R>(
            &client,
            CubeCount::Static(1, 1, 1),
    ...
    for row in 0..m {
        for col in 0..n {
            let mut sum = 0.0;
            for kk in 0..k {
                let block = kk / (k / scales_factor);
                let lhs_val = lhs_data[row * k + kk];
                let lhs_scale = lhs_scales_data[row * scales_factor + block].to_f32();
                let rhs_val = rhs_data[col * k + kk];
                let rhs_scale = rhs_scales_data[col * scales_factor + block].to_f32();
                sum += lhs_val * lhs_scale * rhs_val * rhs_scale;
            }
            expected.push(sum);
        }
...
macro_rules! testgen_cmma {
    () => {
        ...
        #[$crate::runtime_tests::test_log::test]
        fn test_cmma_scaled_fp4_e4m3_factor_4() {
            let client = TestRuntime::client(&Default::default());
            let cube_dimensions = cube_dim::<TestRuntime>(&client);
            cubecl_core::runtime_tests::cmma::test_cmma_scaled_fp4_e4m3_factor_4::<TestRuntime>(
                client,
                cube_dimensions,
            )
        }
