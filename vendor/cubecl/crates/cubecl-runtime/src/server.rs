    ...
    Self: Sized,
{
    ...
    // NOTE (P1): the four graph-capture methods below default to `unimplemented!`, so probing
    // capability means catching a panic. This should become an explicit capability probe (e.g. a
    // `fn supports_capture(&self) -> bool` or returning `Result`/`Option`) so non-CUDA backends can
    // decline cleanly instead of panicking. Deferred to P1 with the capture arena.

    /// Begin a graph-capture ARENA session on `stream_id` (component C2 of the CUDA-graph plan):
    /// install a fresh, growable graph-private arena so every allocation made until
    /// [`Self::capture_end`] / [`Self::capture_abort`] is served from it (recycled within the graph,
    /// never returned to the general pool). Run BEFORE the eager warmup/measure pass; the arena
    /// grows to the closure's peak-live working set during warmup, is locked by
    /// [`Self::capture_arena_lock`], and is sealed to the graph by [`Self::capture_end`].
    ///
    /// Runtimes without graph capture leave the default (no-op) implementation.
    fn capture_arena_begin(&mut self, _stream_id: StreamId) {}

    /// Lock the active capture arena (no further growth) on `stream_id`, right before opening the
    /// CUDA capture window, so the window issues zero device allocations.
    fn capture_arena_lock(&mut self, _stream_id: StreamId) {}

    /// Register a fresh SHARED capture pool (P4 — vLLM `graph_pool_handle`) and return its id. Several
    /// captured graphs of DIFFERENT shapes (e.g. prompt-length buckets) that replay SERIALLY can be
    /// captured into one pool via [`Self::capture_pool_begin`], so they cost ~1 graph's device memory
    /// instead of N×. Returns 0 on runtimes without graph capture (the id is never used there). The
    /// pool is bound to `stream_id`: every capture into it + every replay must stay on this stream.
    fn capture_pool_create(&mut self, _stream_id: StreamId) -> u64 {
        0
    }

    /// Install the shared pool `pool_id` as the active capture arena for the NEXT warmup+capture pass
    /// on `stream_id` (in place of [`Self::capture_arena_begin`]'s fresh per-graph arena). The pass is
    /// sealed back into the pool by [`Self::capture_end`]. Default no-op.
    ///
    /// SAFETY / soundness: only valid for graphs that replay SERIALLY on this one stream — the pool's
    /// blocks are baked into every graph captured into it, so two concurrent replays would clobber
    /// them. The caller must keep all of a pool's captures + replays on a single stream.
    fn capture_pool_begin(&mut self, _pool_id: u64, _stream_id: StreamId) {}

    /// Drop the pool handle's reference to `pool_id` (its [`crate::client::CapturePoolHandle`] was
    /// dropped): frees the pool's device blocks once every graph captured into it is also destroyed.
    /// Default no-op.
    fn capture_pool_release(&mut self, _pool_id: u64, _stream_id: StreamId) {}

    /// Device bytes reserved by the arena sealed to `graph_id` (its peak-live high-water mark), or 0
    /// if unknown / unsupported.
    fn graph_arena_bytes(&mut self, _graph_id: u64, _stream_id: StreamId) -> u64 {
        0
    }

    /// Begin recording all device work issued on `stream_id` into a replayable graph
    /// (CUDA-graph capture, component C1 of the CUDA-graph plan).
    ///
    /// Between [`Self::capture_begin`] and [`Self::capture_end`] the caller must issue only
    /// static-shape, host-sync-free work (no reads, no `CubeCount::Dynamic`). The two calls must
    /// target the same `stream_id`, and the work in between must run on the same stream/thread.
    ///
    /// Runtimes that do not support graph capture leave the default (panicking) implementation.
    fn capture_begin(&mut self, _stream_id: StreamId) {
        unimplemented!("Graph capture is not supported by this runtime.");
    }

    /// End the capture started by [`Self::capture_begin`], instantiate the graph, store it on the
    /// server, and return an opaque id that can be replayed with [`Self::graph_replay`] and freed
    /// with [`Self::graph_destroy`].
    fn capture_end(&mut self, _stream_id: StreamId) -> u64 {
        unimplemented!("Graph capture is not supported by this runtime.");
    }

    /// Abort an in-progress capture started by [`Self::capture_begin`] *without* producing a
    /// replayable graph: take the stream back out of capture mode and discard whatever was recorded
    /// so far. This is the error/unwind path — it must leave the server fully usable (e.g. clear any
    /// `capturing` flag and end the stream's capture). Calling it when no capture is in progress is a
    /// no-op.
    ///
    /// Runtimes that do not support graph capture leave the default (no-op) implementation: there is
    /// nothing to undo because [`Self::capture_begin`] would have panicked.
    fn capture_abort(&mut self, _stream_id: StreamId) {}

    /// Replay a previously captured graph on `stream_id` with a single host call.
    fn graph_replay(&mut self, _graph_id: u64, _stream_id: StreamId) {
        unimplemented!("Graph capture is not supported by this runtime.");
    }

    /// Destroy a previously captured graph, releasing its device resources.
    fn graph_destroy(&mut self, _graph_id: u64) {
        unimplemented!("Graph capture is not supported by this runtime.");
    }
}
